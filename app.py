...
# =========================================================
# Open positions / current holdings analysis
# =========================================================

def build_open_positions(trades, prices):
    """Reconstruct currently open positions from order history using FIFO lots.

    This analyzes BUY rows that have not been fully matched by SELL rows.
    It adds current price, unrealized P&L, max gain since entry, give-back, and current stock regime.
    """
    if trades is None or trades.empty:
        return pd.DataFrame()

    lots = {}
    for _, r in trades.sort_values(["date", "datetime"]).iterrows():
        ticker = r["ticker"]
        side = r["side"]
        qty = float(r["quantity"])
        price = float(r["price"])
    ...
    price_indicators = build_price_indicators(prices) if prices is not None and not prices.empty else pd.DataFrame()
    rows = []
    for ticker, ticker_lots in lots.items():
        for lot in ticker_lots:
            qty = float(lot.get("remaining_qty", 0) or 0)
            if qty <= 0:
                continue

            buy_price = float(lot["buy_price"])
            cost_per_share = buy_price + float(lot.get("buy_fee_per_share", 0) or 0)
            buy_date = lot["buy_date"]
            ...
            row = {
                ...
                "cost_value": cost_per_share * qty,
                "current_price": np.nan,
                "current_value": np.nan,
                "unrealized_pnl": np.nan,
                "unrealized_return": np.nan,
                "max_price_since_buy": np.nan,
                "max_gain_since_buy": np.nan,
                "current_capture_rate": np.nan,
                "giveback_from_peak": np.nan,
                "holding_days_open": np.nan,
                "current_stock_regime": "No price data",
                "current_entry_type": "No price data",
                "current_confidence": "Very low",
                "latest_price_date": pd.NaT,
                "current_ma20": np.nan,
                "current_ma60": np.nan,
                "current_ma200": np.nan,
                "current_return_20d": np.nan,
                "current_price_vs_ma20": np.nan,
                "current_price_vs_ma60": np.nan,
                "current_price_vs_ma200": np.nan,
                "current_volume_vs_ma20": np.nan,
            }

            if not price_indicators.empty:
                tp = price_indicators[(price_indicators["ticker"] == ticker) & (price_indicators["date"] >= buy_date)].sort_values("date")
                latest_ctx = price_indicators[price_indicators["ticker"] == ticker].sort_values("date")

                if not tp.empty:
                    latest = tp.iloc[-1].to_dict()
                    current_price = float(latest.get("close"))
                    max_price = float(tp["close"].max())
                    row.update({
                        "latest_price_date": latest.get("date"),
                        "current_price": current_price,
                        "current_value": current_price * qty,
                        "unrealized_pnl": (current_price - cost_per_share) * qty,
                        "unrealized_return": current_price / cost_per_share - 1 if cost_per_share else np.nan,
                        "max_price_since_buy": max_price,
                        "max_gain_since_buy": max_price / cost_per_share - 1 if cost_per_share else np.nan,
                        "holding_days_open": (latest.get("date") - buy_date).days if pd.notna(latest.get("date")) else np.nan,
                    })

                    max_gain = row["max_gain_since_buy"]
                    cur_gain = row["unrealized_return"]
                    if pd.notna(max_gain) and max_gain > 0 and pd.notna(cur_gain):
                        row["current_capture_rate"] = max(cur_gain, 0) / max_gain
                        row["giveback_from_peak"] = max_gain - cur_gain

                if not latest_ctx.empty:
                    ctx = latest_ctx.iloc[-1].to_dict()
                    row.update({
                        "current_context_price_date": ctx.get("date"),
                        "current_ma20": ctx.get("ma20"),
                        "current_ma60": ctx.get("ma60"),
                        "current_ma200": ctx.get("ma200"),
                        "current_return_20d": ctx.get("return_20d"),
                        "current_price_vs_ma20": ctx.get("price_vs_ma20"),
                        "current_price_vs_ma60": ctx.get("price_vs_ma60"),
                        "current_price_vs_ma200": ctx.get("price_vs_ma200"),
                        "current_volume_vs_ma20": ctx.get("volume_vs_ma20", np.nan),
                        "current_stock_regime": _classify_market_regime(ctx),
                        "current_entry_type": _classify_entry_type(ctx),
                        "current_confidence": _classification_confidence(ctx),
                    })
    ...
    agg = out.groupby("ticker", as_index=False).agg(
        ...
        max_price_since_buy=("max_price_since_buy", "max"),
        current_price=("current_price", "last"),
        current_stock_regime=("current_stock_regime", "last"),
        current_entry_type=("current_entry_type", "last"),
        current_confidence=("current_confidence", "last"),
        current_ma20=("current_ma20", "last"),
        current_ma60=("current_ma60", "last"),
        current_ma200=("current_ma200", "last"),
        current_return_20d=("current_return_20d", "last"),
        current_price_vs_ma20=("current_price_vs_ma20", "last"),
        current_price_vs_ma60=("current_price_vs_ma60", "last"),
        current_price_vs_ma200=("current_price_vs_ma200", "last"),
        current_volume_vs_ma20=("current_volume_vs_ma20", "last"),
    )
    agg["avg_cost_per_share"] = agg["cost_value"] / agg["quantity"]
    agg["unrealized_return"] = agg["current_value"] / agg["cost_value"] - 1
    agg["max_gain_since_buy"] = agg["max_price_since_buy"] / agg["avg_cost_per_share"] - 1
    agg["current_capture_rate"] = np.where(
        agg["max_gain_since_buy"] > 0,
        np.maximum(agg["unrealized_return"], 0) / agg["max_gain_since_buy"],
        np.nan
    )
    agg["giveback_from_peak"] = agg["max_gain_since_buy"] - agg["unrealized_return"]
    agg["holding_days_open"] = (pd.to_datetime(agg["latest_price_date"]) - pd.to_datetime(agg["first_buy_date"])).dt.days

    preferred_cols = [
        "ticker", "quantity", "first_buy_date", "latest_price_date",
        "avg_cost_per_share", "current_price", "cost_value", "current_value",
        "unrealized_pnl", "unrealized_return", "max_price_since_buy", "max_gain_since_buy",
        "current_capture_rate", "giveback_from_peak", "holding_days_open",
        "current_stock_regime", "current_entry_type", "current_confidence",
        "current_ma20", "current_ma60", "current_ma200", "current_return_20d",
        "current_price_vs_ma20", "current_price_vs_ma60", "current_price_vs_ma200", "current_volume_vs_ma20"
    ]
    return agg[[c for c in preferred_cols if c in agg.columns]].sort_values("current_value", ascending=False)
...
open_positions = build_open_positions(trades, prices)
...
with tab5:
    ...
    if prices.empty:
        ...
        render_heatmap_table(pivot_auto)
...
with tab10:
    st.subheader("Open Positions: currently held BUY lots")
    st.caption("This tab reconstructs positions that have been bought but not fully sold using FIFO. It estimates unrealized P&L, max gain since entry, give-back from peak, and current stock regime from the latest available price data.")

    if open_positions.empty:
        st.info("No open positions detected from the uploaded order history, or price data is unavailable for current-position analysis.")
        st.markdown("""
Possible reasons:
- All BUY quantities have been fully matched by SELL rows.
- The uploaded file does not include your latest BUY trades.
- Price history is missing for remaining tickers.
""")
    else:
        total_cost = open_positions["cost_value"].sum(skipna=True) if "cost_value" in open_positions.columns else np.nan
        total_value = open_positions["current_value"].sum(skipna=True) if "current_value" in open_positions.columns else np.nan
        total_unrealized = open_positions["unrealized_pnl"].sum(skipna=True) if "unrealized_pnl" in open_positions.columns else np.nan
        total_unrealized_return = total_value / total_cost - 1 if pd.notna(total_cost) and total_cost != 0 else np.nan

        oc1, oc2, oc3, oc4 = st.columns(4)
        oc1.metric("Open cost value", fmt_vnd(total_cost))
        oc2.metric("Current value", fmt_vnd(total_value))
        oc3.metric("Unrealized P&L", fmt_vnd(total_unrealized))
        oc4.metric("Unrealized return", fmt_pct(total_unrealized_return))

        st.markdown("#### Current holdings health")
        display_cols = [
            "ticker", "quantity", "first_buy_date", "latest_price_date",
            "avg_cost_per_share", "current_price", "cost_value", "current_value",
            "unrealized_pnl", "unrealized_return", "max_price_since_buy",
            "max_gain_since_buy", "current_capture_rate", "giveback_from_peak",
            "holding_days_open", "current_stock_regime", "current_price_vs_ma20",
            "current_price_vs_ma60", "current_price_vs_ma200", "current_volume_vs_ma20"
        ]
        display_cols = [c for c in display_cols if c in open_positions.columns]
        open_display = open_positions[display_cols].copy()
        non_numeric_open = [c for c in open_display.columns if not pd.api.types.is_numeric_dtype(open_display[c])]
        render_heatmap_table(open_display, exclude_cols=non_numeric_open)

        if "unrealized_pnl" in open_positions.columns:
            st.markdown("#### Unrealized P&L by ticker")
            chart_df = open_positions.dropna(subset=["unrealized_pnl"]).sort_values("unrealized_pnl", ascending=False)
            if not chart_df.empty:
                fig_open_pnl = px.bar(chart_df, x="ticker", y="unrealized_pnl", title="Unrealized P&L by open position")
                st.plotly_chart(fig_open_pnl, use_container_width=True)

        if "current_stock_regime" in open_positions.columns:
            st.markdown("#### Exposure by current stock regime")
            regime_exposure = open_positions.groupby("current_stock_regime", as_index=False).agg(
                positions=("ticker", "count"),
                current_value=("current_value", "sum"),
                unrealized_pnl=("unrealized_pnl", "sum"),
                avg_unrealized_return=("unrealized_return", "mean"),
            )
            render_heatmap_table(regime_exposure, exclude_cols=["current_stock_regime"])

        if "giveback_from_peak" in open_positions.columns:
            st.markdown("#### Give-back from peak")
            gb = open_positions.dropna(subset=["giveback_from_peak"]).sort_values("giveback_from_peak", ascending=False).head(20)
            if not gb.empty:
                fig_gb = px.bar(gb, x="ticker", y="giveback_from_peak", title="Give-back from max gain since buy")
                fig_gb.update_yaxes(tickformat=".0%")
                st.plotly_chart(fig_gb, use_container_width=True)

        csv_open = open_positions.to_csv(index=False).encode("utf-8-sig")
        st.download_button(
            label="Download open positions CSV",
            data=csv_open,
            file_name="open_positions_analysis.csv",
            mime="text/csv"
        )
